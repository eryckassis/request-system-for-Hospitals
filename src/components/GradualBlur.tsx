  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[`mobile${k}` as keyof GradualBlurProps]) {
        v = config[`mobile${k}` as keyof GradualBlurProps];
      } else if (w <= 768 && config[`tablet${k}` as keyof GradualBlurProps]) {
        v = config[`tablet${k}` as keyof GradualBlurProps];
...
const injectStyles = () => {
  ...
  el.id = id;
  el.textContent = ".gradual-blur{pointer-events:none;transition:opacity .3s ease-out}";
